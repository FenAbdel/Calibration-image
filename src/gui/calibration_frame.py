...
class CalibrationFrame(tb.Frame):
    ...
    def on_camera_name_change(self, event=None):
        """Handle camera name changes and create/switch to appropriate folder"""
        ...
        if os.path.exists(self.current_camera_folder):
            ...
            for idx, filename in enumerate(image_files):
                ...
                try:
                    ...
                    btn = tb.Button(self.images_frame, image=photo, 
                                  command=lambda fp=full_path: self.validate_image(fp),
                                  bootstyle="link")
    ...
    def start_capture_server(self):
        camera_name = self.camera_name_entry.get().strip()
        if not camera_name:
            messagebox.showerror("Error", "Please enter a camera name first.")
            return
        ...
        tb.Label(qr_window, text="Scan this QR code with your phone to capture calibration images:", 
                font=("Segoe UI", 12, "bold")).grid(row=0, column=0, padx=10, pady=10)
